...
const NuevoFormulario = ({setPageTitle}) => {
    ...
    const validarRFC = (rfc) => {
        // RFC para persona moral: 3 letras + 6 dígitos + 3 caracteres alfanuméricos
        // RFC para persona física: 4 letras + 6 dígitos + 3 caracteres alfanuméricos
    ...
    const handleSubmit = async (e) => {
        ...
        if (formData.email_empresa && !validarEmail(formData.email_empresa)) {
            errors.email_empresa = "Formato de email inválido";
        }

        // Validación específica de RFC (solo si se proporciona)
        if (formData.rfc_empresa && formData.rfc_empresa.trim() !== "" && !validarRFC(formData.rfc_empresa)) {
            errors.rfc_empresa = "Formato de RFC inválido (ej: ABC123456XYZ)";
        }
        ...
        // Validación de estructura organizacional (obligatorio)
        const tieneEstructura = puestosSeleccionados.length > 0 || puestosExtra.some(p => p.nombre.trim() !== "");
        if (!tieneEstructura) {
            errors.estructura = "Debe seleccionar al menos un puesto en la estructura organizacional";
        }

        // Validación de adscripciones (obligatorio)
        const tieneAdscripciones = adscripcionesSeleccionadas.length > 0 || adscripcionesExtra.some(a => a.trim() !== "");
        if (!tieneAdscripciones) {
            errors.adscripciones = "Debe seleccionar al menos una adscripción";
        }
                    ...
                    <GlassCard>
                        ...
                        <Box component="form" display="flex" flexDirection="column" gap={3}>
                            ...
                            <TextField
                                ...
                                onChange={handleChange}
                                error={!!formErrors.rfc_empresa}
                                helperText={formErrors.rfc_empresa || "Formato: ABC123456XYZ (opcional)"}
                                            ...
                                            control={
                                                <Checkbox
                                                    ...
                                                    onChange={(e) => {
                                                        ...
                                                        if (checked && formErrors.estructura) {
                                                            setFormErrors(prev => ({ ...prev, estructura: undefined }));
                                                        }
                                                    }}
                                                />
                                            }
                                    ...
                                    <Stack direction="column" spacing={1} flex={1}>
                                        <TextField
                                            ...
                                            onChange={(e) => {
                                                ...
                                                // Limpiar error cuando se escribe algo
                                                if (nuevoNombre.trim() !== "" && formErrors.estructura) {
                                                    setFormErrors(prev => ({ ...prev, estructura: undefined }));
                                                }
                                            }}
                                            placeholder="Nombre"
                                                ...
                                                control={
                                                    <Checkbox
                                                        ...
                                                        onChange={(e) => {
                                                            ...
                                                            // Limpiar error cuando se selecciona algo
                                                            if (checked && formErrors.adscripciones) {
                                                                setFormErrors(prev => ({ ...prev, adscripciones: undefined }));
                                                            }
                                                        }}
                                                    />
                                                }
                                    ...
                                    <Stack key={`ads-extra-${idx}`} direction="row" alignItems="center" spacing={2}>
                                        <TextField
                                            ...
                                            onChange={(e) => {
                                                ...
                                                // Limpiar error cuando se escribe algo
                                                if (val.trim() !== "" && formErrors.adscripciones) {
                                                    setFormErrors(prev => ({ ...prev, adscripciones: undefined }));
                                                }
                                            }}
